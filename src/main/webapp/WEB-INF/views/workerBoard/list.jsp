...
<div class="container">
			 ...
			 <!-- 상세검색 -->
		  	<div id="detail" >
		  		<div style="position: relative; left:12px;"><b>상세검색</b></div>
		  			<form method="POST" action="${pageContext.request.contextPath}/workerBoard/detail">
						  		<fieldset>
									<legend></legend>
							...
							<button class="btn btn-primary" id="allListbtn" 
								onclick="location.href='<c:url value="/workerBoard/list" />'">초기화</button>
		   </div> 
		   <!-- 상세검색 끝 -->
   ...
   <div id="blk"></div> 
   		...
   		<div style="text-align:right;">
			<c:if test="${!empty myuserCode}">
				<a type="button" class="btn btn-primary" style="background-color:#1ABC9C;border-color:#1ABC9C;" 
					href="<c:url value="/workerBoard/write" />">글쓰기</a>
			</c:if>	
		</div>
	...
	<!-- 글 목록 없다면  -->
   	<c:if test="${empty workerBoardList}"> 							
   	...
   	<c:if test="${!empty workerBoardList}">
   		<div class="row">	 
			<table class="table" style="text-align: center; border:1px solid #dddddd">
	            ...
	            <tbody> 
	                <c:forEach var="workerBoard" items="${workerBoardList}" varStatus="loop">
	                        <tr>
	                        	<td>
	                        					<!-- DB에 char형식으로 저장돼있어서 .charAt(0) 붙여줘야함 -->
	                        		<c:if test = "${workerBoard.timeover eq 'Y'.charAt(0)}">
	                        			<img src="${pageContext.request.contextPath }/image/timeover2.png" width="25">
	                        		</c:if>
	                        		</td>
	                            <td id="wbCode">${workerBoard.wbCode}</td>	
	                            <td class="dropdown">
	     	                      	<a href="#" class="dropdown-toggle" 
	     	                      		data-toggle="dropdown" aria-haspopup="true"
										aria-expanded="false">${workerBoard.userCode}</a>
								   		<ul class="dropdown-menu">
									        <li><a href="${pageContext.request.contextPath}/myBlog/main/${workerBoard.userCode}">회원블로그</a></li>
											<li><a href="${pageContext.request.contextPath}/chat/${workerBoard.userCode}"
										       		onclick="window.open(this.href,'팝업창','width=466,height=620'); return false;">채팅하기</a></li>
									       	<li><a href="#" id="lover" onclick="lover('${myuserCode}', '${workerBoard.userCode}'); return false;" >찜하기</a></li>
									       	<li><a href="${pageContext.request.contextPath}/qnaStep1">신고하기</a></li>
								       	</ul>
	     	                      	</td> 
	                            <td><a href="<c:url value="/workerBoard/read/${workerBoard.wbCode}"/>">${workerBoard.title}</a></td>
	                            <td>${workerBoard.regDate}</td>       
	                            <td>${workerBoard.count}</td>
	                        </tr>
	                </c:forEach>
	        	</tbody>
	        </table>
	        ...
	        <!-- 검색 -->
			<div style="float:right;" class="wbSearch" >
			 	<select style="width: auto; float:left;" class="form-control" name="searchType">
			 		<%-- <option value="n"<c:out value="${wbs.searchType == null ? 'selected' : ''}"/>>---검색---</option> --%>
					<option value="t"<c:out value="${wbs.searchType eq 't' ? 'selected' : ''}"/>>제목</option>
			 		<option value="a"<c:out value="${wbs.searchType eq 'a' ? 'selected' : ''}"/>>주소</option>
			 		<option value="j"<c:out value="${wbs.searchType eq 'j' ? 'selected' : ''}"/>>업직종</option>
			 		<option value="wt"<c:out value="${wbs.searchType eq 'wt' ? 'selected' : ''}"/>>근무시간대</option>
			 	</select>
			 		<input style="width: auto; float:left;" class="form-control" type="text" name="keyword" id="keywordInput" value="${wbs.keyword }"/> 	
	        ...
	        <div style="text-align:center;">
				<ul class="pagination">
					<c:if test="${wbPageMaker.prev}">
						<li><a href="list${wbPageMaker.makeSearch(wbPageMaker.startPage - 1)}"><</a></li>
					</c:if> 
					 
					<c:forEach begin="${wbPageMaker.startPage}" end="${wbPageMaker.endPage}" var="idx">
						<li><a href="list${wbPageMaker.makeSearch(idx)}">${idx}</a></li>
					</c:forEach>
							    
					<c:if test="${wbPageMaker.next && wbPageMaker.endPage > 0}">
						<li><a href="list${wbPageMaker.makeSearch(wbPageMaker.endPage + 1)}">></a></li>
					</c:if> 
				</ul>
			</div>
			<!-- 페이징 끝 -->
