...
<c:if test="${message == null}">
	...
	<section>
		<div>
			<h1 style="padding-top: 10px; padding-bottom: 10px; padding-left: 15%; padding-right: 15%; text-align: left;">1:1 문의 관리</h1>
		</div>
		<div>
			<table class="table" style="margin:0 auto;">
				...
				<c:forEach items="${dto}" var="dto">
					<tr>
						<td><input type="checkbox" id=check name="check" value="${dto.questionNo}"></td>
						<td>${dto.questionNo}</td>
						<td>${dto.inquiryType}</td>
						<td><a href="${pageContext.request.contextPath}/admin/qna/viewContent?questionNo=${dto.questionNo}">${dto.title}</a></td>
						<td>${dto.writerCode}</td>
						<td>${dto.regDate}</td>
						<td>${dto.yn}</td>
					<tr>
				</c:forEach>
					<tr>
						<td colspan="7" align="right" style="border-top: none;">
							<input type="button" class="btn btn-outline-button btn-round" id="delete" onclick="deleteContent()" value="삭제">
						</td>
					</tr>
				</tbody>
			</table>
		</div>
		<div aria-label="Page navigation example" style="padding-left: 900px; padding-right: 45%; padding-top: 80px; padding-bottom: 0px;">
			<ul class="pagination">
			<c:if test="${pageMaker.prev}">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/admin/qna?page=${pageMaker.startPage - 1}">&lt</a>
				</li>
			</c:if>
			<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
  				<li><a href="${pageContext.request.contextPath}/admin/qna?page=${idx}">${idx}</a></li>
			</c:forEach>
			<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
				<li class="page-item">
					<a class="page-link" href="${pageContext.request.contextPath}/admin/qna?page=${pageMaker.endPage + 1}">&gt</a>
				</li>
			</c:if>
			</ul>
		</div>		
...
<script>
	...
	function deleteContent() {
		...
		if(questionNo != null) {
			...
			var data = questionNo;
			$.ajax({
				url: "${pageContext.request.contextPath}/admin/qna/deleteContent",
				type: "POST",
				data: questionNo,
