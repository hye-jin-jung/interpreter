...
<c:if test="${message == null}">
	...
	<section>
		<div>
			<h1 style="padding-top: 10px; padding-bottom: 10px; padding-left: 15%; padding-right: 15%; text-align: left;">공지사항 관리</h1>
		</div>
		<div>
		<form name="form">
			<table class="table" style="margin:0 auto;">
				...
				<c:forEach items="${dto}" var="dto">
					<tr>
						<td><input type="checkbox" id=check name="check" onclick="checkBox(form.check, this)" value="${dto.no}"></td>
						<td>${dto.no}</td>
						<td><a href="${pageContext.request.contextPath}/admin/notice/viewContent?no=${dto.no}">${dto.title}</a></td>
						<td>${dto.regDate}</td>
					<tr>
				</c:forEach>
				</tbody>
				<tfoot>
					<tr>
						<td colspan="4" align="right">
							<button type="button" class="btn btn-outline-button btn-round" onclick="location.href='${pageContext.request.contextPath}/admin/notice/writeContent'">글쓰기</button>
	...
	<%-- <footer style="width: 100%;">
		...
		<br>
		<div class="footer" style="width: 100%; text-align: center; margin: auto;">
...
<script>
	...
	function chkBoxCheck(intChkNumber) {
		...
		출처: https://luckys.tistory.com/37 [Lucky's...]
	...
	function deleteContent() {
		...
		if(noticeNo != null) {
			...
			$.ajax({
				url: "${pageContext.request.contextPath}/admin/notice/deleteContent",
				type: "POST",
				data: noticeNo,
