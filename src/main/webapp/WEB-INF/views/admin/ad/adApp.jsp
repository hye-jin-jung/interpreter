...
<c:if test="${message == null}">
	...
	<section>
		<div>
			<h1 style="padding-top: 10px; padding-bottom: 10px; padding-left: 15%; padding-right: 15%; text-align: left;">광고 문의 관리</h1>
		</div>
		<div>
			<table class="table" style="margin:0 auto;">
				...
				<c:forEach items="${vo}" var="vo">
					<tr>
						<td><input type="checkbox" id=check name="check" onclick="checkBox(form.check, this)" value="${vo.pageNo}"></td>
						<td>${vo.pageNo}</td>
						<td>${vo.adCategory}</td>
						<td>
							<a href="${pageContext.request.contextPath}/admin/adApp/viewContent?pageNo=${vo.pageNo}">${vo.title}</a>
							<a href="${pageContext.request.contextPath}/admin/adApp/viewContent?pageNo=${vo.pageNo}">${vo.enterprise}</a>
						</td>
						<td>${vo.userCode}</td>
						<td>${vo.regDate}</td>
					<tr>
...
<script>
	...
	function deleteContent() {
		...
		if(pageNo != null) {
			...
			var data = pageNo;
			$.ajax({
				url: "${pageContext.request.contextPath}/admin/adApp/deleteContent",
				type: "POST",
				data: pageNo,
