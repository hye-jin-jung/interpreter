...
<html>
<head>
<title>${employerBoardVO.title}&nbsp 수정</title>
...
<div class="container">
	<div class="row">	
		<h2>일나눔 글 수정</h2>
		<form:form name="form" id="form" commandName="employerBoardVO" method="POST" onsubmit="return writeCheck()">
			<div class="form-group">
					<form:input style="text-align:center; height:60px; font-size:26px;" class="form-control" path="title" name="title"/> 
					<form:errors path="title" />
				
				<table class="table" style="text-align:left;"> 
					<tr>
						<th><form:label path="ebCode">게시글코드</form:label></th>
						<td><form:input style="width: 30%;" class="form-control" path="ebCode" type="text" value="${ebCode }" readOnly="true"/>
						<form:errors path="userCode" /></td>
					</tr>		
		  			<tr>
						<th><form:label path="address">주소</form:label></th>
						<td><form:input path="address" class="form-control" type="text" id="roadFullAddr"  name="roadFullAddr" placeholder="주소를 입력하세요." readOnly="true" /> 
						<form:input path="address" type="button" onClick="goPopup();" value="주소 검색"/>
					...
					<tr>
						...
						<textarea class="form-control" name="content" rows="13" cols="50" wrap="hard">${employerBoardVO.content}</textarea> 
						<form:errors path="content" />
			</div>
			<div style="text-align:center;">
				<input type="submit" class="btn btn-primary" style="background-color:#1ABC9C;border-color:#1ABC9C;" value="수정"> 
				<a type="button" class="btn btn-primary" style="background-color:#1ABC9C;border-color:#1ABC9C;"
					href="<c:url value="/employerBoard/read/${ebCode}" />">취소</a>
			</div>
		</form:form>
	</div>
</div>
